...
- R2 proves only `read_file`, `search_text`, and terminal `finish_review`; R3 completes the initial live profile with `list_changed_files`, `read_diff`, and `list_files`.
- Agent/core owns canonical logical records, stable-prefix and dynamic-suffix classification, the provider-neutral logical request plan, logical prefix identity, loop, budgets, tools, and terminal semantics. The provider adapter owns provider-specific role/message/block/tool projection, continuation placement, request-envelope and wire serialization, any provider-specific cache-relevant identity, HTTP transport, and usage/error mapping. Neither a generic AI abstraction nor an SDK may silently reorder or rewrite the cache-relevant projection.
...
- Repository, pull-request, diff, file, search-result, and tool-result content always remains untrusted data. Its durable record kind, provider-facing role or framing, tool-call association, and control/data classification are fixed by Host/Agent code, authenticated as session structure, preserved across restoration and provider materialization, and never promoted into policy, system/developer instructions, tool definitions, provider configuration, endpoint selection, secret channels, or other control records.
