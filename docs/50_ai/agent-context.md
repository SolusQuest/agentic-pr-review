...
R1 is complete: the legacy mixed Action, TypeScript coordinator, and Claude Code CLI execution path are gone. R4 now includes a nested generated Node 24 wrapper for repository-controlled prepared-payload proof, but it is not a supported downstream Action before R7. W3 removed the retained TypeScript invocation families and obsolete live-context schema after W2/W7/E1 replacement proof; W10 removed the uncalled TypeScript review protocol/structured-result family while retaining the embedded Input/Result/Trace schemas and complete C#-owned fixture corpus; W14 removed the TypeScript canonicalization family after its behavior and test dispositions were pinned to C# owners; and W15 removed the consumerless root `src/types.ts` and `src/utils.ts` modules after checking every exported symbol and historical W7-W10 consumer. R4-W14 retired the TypeScript canonical-json family; C# Canonical remains current and the prefix corpus remains immutable evidence. The direct-runtime schemas/models remain live C# contracts and are distinct from ActionHost replacement behavior. Read `docs/20_architecture/r1-legacy-removal-handoff.md` before changing retained protocol, state, publisher, ledger, or artifact-provenance evidence.
