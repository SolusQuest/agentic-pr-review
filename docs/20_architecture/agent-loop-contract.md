...
The raw argument string on a current provider response has one narrower spelling boundary before it becomes a logical record. Agent/core parses it within the existing nonterminal or terminal byte cap, rejects duplicate or structurally invalid JSON, normalizes mathematically integral in-range number tokens to Int32 without binary64 rounding, deserializes the temporary value through the closed source-generated tool DTO, applies every existing required/optional-property and domain rule, and compares it by bounded JSON value semantics with the exact provider shapes emitted by that tool's project writer. This permits only insignificant whitespace, object-property order, and equivalent string or bounded-Int32 number spelling for an otherwise admitted current call. Fractional, near-integer, out-of-range, unknown or missing fields, explicit null for omitted-only optionals, wrong kinds, invalid domains, trailing content, and structural-cap failures remain rejected. The temporary normalization and generic comparison are not retained representations or wider-integer compatibility readers; the existing schema-order writer immediately produces the only admitted bytes.
