...
Byte-for-byte stability is defined over the runtime-owned append-safe canonical
provider-prefix segment stream (deterministic key order, UTF-8, newline
normalization, default-value omission, array ordering), not over raw HTTP
bodies, provider SDK internal objects, or tokenized prefixes. The exact
segment framing and hash domain are normative in
