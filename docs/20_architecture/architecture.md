...
This is the selected target direction. R1 removed the TypeScript business host and Claude Code CLI path. R2 established the project-minimal Agent product path: the selected chat seam, explicit loop and tools, current SESSION format, restricted encrypted STATE, and two-fresh-process framework and Linux x64 Native AOT proof. R3 then proved the real Agent transport path and removed the superseded single-request C# application and executor family; see the checked [`r3-single-shot-removal-handoff.md`](./r3-single-shot-removal-handoff.md). R4 now includes the nested generated Node 24 wrapper and official artifact bridge for repository-controlled prepared-payload proof. That development-head surface is not a supported downstream Action; R7 still owns release payload delivery and the public default. The earlier two-candidate comparison infrastructure is also deleted; see [`ai-abstraction-decision.md`](./ai-abstraction-decision.md).
