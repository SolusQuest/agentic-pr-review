...
Selection occurs before the #55 invocation. `StateSelectionSnapshot` retains the exact observed selector bytes, exact predecessor bytes, and host-authoritative current head/base/base-ref facts in memory; its ID hashes those facts together with the branch semantic envelope and specified byte hashes. It is never upgraded by rereading the selector during acceptance. Missing automatic state selects bootstrap. Conclusively corrupt or unavailable referenced state selects a recovery root with closed recovery evidence; transport or transaction failures return infrastructure failure and never synthesize bootstrap or recovery. Explicit invalid state fails closed.

Acceptance creates an immutable snapshot while holding the per-state-key transaction. It verifies the observed selector revision, captures a decimal sequence cutoff, freezes every matching registration at or below that cutoff, and returns a completeness assertion from that same store transaction containing the matching count and exact matching registration bytes. The candidate-set digest covers that assertion and the compact ordered projection, and acceptance rejects a missing or internally inconsistent response before classification. `StateAcceptanceStore` is the completeness trust boundary: acceptance can detect response mutation or a self-inconsistent projection, but it cannot independently prove that a faulty backend did not omit records and then recompute a coherent assertion and digest. Every production store must therefore enumerate the complete matching set atomically before returning; it must not treat a paginated projection as complete. The hard limits are exactly 64 matching registrations and 2,097,152 aggregate registration bytes. Exceeding either limit returns `candidate_snapshot_limit_exceeded` with no partial snapshot or digest. Duplicate IDs, duplicate sequences, invalid sequence fields, and sequence overflow fail closed; numeric gaps are allowed.
