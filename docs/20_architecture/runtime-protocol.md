...
R4-W15 subsequently removed the consumerless root `src/types.ts` and `src/utils.ts` modules. The W15 record classifies all 31 type exports and 21 helpers exactly once and pins the fourteen historical W7-W10 importers to the accepted E1 base. Direct-runtime fields such as provider, phase, tool mode, inline policy, changed files, findings, context documents, and current-run usage remain owned by these embedded schemas, C# models, `RuntimeApplication`, `RuntimeJson`, and protocol tests; ActionHost owns its separately grounded and trusted equivalents. The schema descriptions now name `SchemaContracts` rather than the deleted helper, with no validation-keyword or version change.
