...
Status: normative R2 current-format contract implemented by issue [#89](https://github.com/SolusQuest/agentic-pr-review/issues/89).
...
| `tool_call`            | identifier `call_id`, name `read_file` or `search_text`, canonical `arguments_json` of 1-8 KiB                           |
...
A non-terminal assistant message contains zero or more text/continuation slots and one through eight tool calls, with no terminal call. It is followed immediately by one ordered `tool_result` record per physical call. Each result binds the same assistant message ID and call ID. `arguments_json` must equal the normalized canonical bytes returned by its closed parser; provider-input spellings with omitted/defaulted properties are never retained as a second durable form. `result_json` is the exact canonical admitted read/search result and is at most 32 KiB.
...
On completion of #89, #87 receives the next edit right for `AgentLimits.cs`, Host-facing session plaintext encode/validate interfaces, runtime/test project files, and session/security documentation links. STATE may authenticate, encrypt, store, select, and hand plaintext bytes to SESSION, but it may not reinterpret records or continuation semantics. [#88](https://github.com/SolusQuest/agentic-pr-review/issues/88) retains the fresh-process executable fixture, synthetic transport oracle, final Native AOT comparison, CI workflow, and comparison-fixture deletion.
