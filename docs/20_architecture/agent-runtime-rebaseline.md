...
R4 owns the retained deterministic CLI/application/protocol/storage path, C# `Ledger/**` and `Prefix/**`, TypeScript state-v2/publisher families, GitHub reads and publication, production restricted-state transport and keys, the thin Node bridge, and the final public wrapper. W3 removed `src/runtime-invocation/**`, `src/live-runtime-invocation/**`, and the obsolete live-context schema after W2/W7/E1 replacement proof; W6 removed the superseded Git-ref state-acceptance family and its dedicated schemas after S1-S6/P5/P6/E1 proof. The remaining families are not R2 comparison infrastructure and stay only for their named migration consumers.
