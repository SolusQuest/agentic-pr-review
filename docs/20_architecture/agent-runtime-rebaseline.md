...
R4 owns the retained deterministic CLI/application/protocol/storage path, C# `Ledger/**` and `Prefix/**`, TypeScript state-v2/state-acceptance/publisher families, GitHub reads and publication, production restricted-state transport and keys, the thin Node bridge, and the final public wrapper. W3 removed `src/runtime-invocation/**`, `src/live-runtime-invocation/**`, and the obsolete live-context schema after W2/W7/E1 replacement proof; the remaining families are not R2 comparison infrastructure and stay only for their named migration consumers.
