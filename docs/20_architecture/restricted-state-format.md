...
R4 adds a repository-level encrypted locator-root sentinel and derives scoped artifact names from the stable root rather than directly from the rotatable encryption key. The fixed generic sentinel name reveals product use but no scope mapping. A maximum of eight live sentinel artifacts form one authenticated generation/successor chain; semantically equivalent same-root duplicates are idempotent, distinct roots or incomparable successors conflict, and each selected head authenticates the exact identities and digests it supersedes. Rotation and refresh append/read back one current-key successor, prune exact superseded artifacts, and preserve the root without another configured secret. The previous key remains read-only for every unexpired state or transaction record needed for recovery and is removed only after none remains. Every scoped artifact must expire at least 24 hours before the selected sentinel; if metadata cannot prove that invariant, absence is `state_locator_unavailable`, not bootstrap.
