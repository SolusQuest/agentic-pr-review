...
The class retains the accepted generation and its immediate predecessor for at most seven days from a Host-trusted acceptance timestamp. Restore enumeration exposes only accepted objects, ordered by generation descending and then envelope SHA-256 ordinal, and at most two candidates. One separate staging slot may hold the next prepared envelope. Staging is never a restore candidate. Accepted candidate metadata is at most 16 KiB, accepted envelope bytes are at most 4 MiB, and accepted plus staging envelope bytes are at most 6 MiB. Partial, malformed, out-of-order, or over-limit enumeration fails closed and selects no partial set.
...
`PreparedStateReceipt` contains generation, session SHA-256, envelope SHA-256, and exact prepared object identity. Encryption occurs once per prepare. Outcome-unknown reconciliation reuses the exact receipt and envelope bytes; it never re-encrypts the same generation with a fresh nonce. A same-generation operation is idempotent only when session hash, envelope hash, and object identity match. The same semantic plaintext encrypted with another nonce is a conflict, not another accepted object.
...
| explicit trusted reset/delete                | `reset/state_reset`                 | atomically remove accepted set and staging                |
| trusted expiry cleanup                       | `reset/state_expired`               | atomically remove expired accepted set and staging        |
...
6. key-ID length UInt16 little-endian plus a 1-64-byte printable ASCII key ID;
...
Compare-and-swap is serialized per scope across local store instances. While holding commit exclusion, the store reads and validates the expected snapshot version, writes a same-directory unique temporary file, flushes data to disk, and atomically creates or replaces the complete scope snapshot. The replacement already contains the new accepted set, retained predecessor, pruning, and staging removal. Stale versions conflict. Cancellation or failure before replacement leaves the previous snapshot visible. Cancellation observed after replacement returns committed success. Linux directory metadata is synchronized after publication where supported.
...
Deterministic tests pin authorization denial, the exact action/code taxonomy, the 332-byte AAD golden, framing and bounds, nonce uniqueness, header/binding/nonce/ciphertext/tag tamper rejection, unknown and wrong keys, approved-key rotation, SESSION semantic re-admission, independent lineage, staging isolation, missing-current no-fallback, expiry-before-key behavior, replay/conflict distinction, exact receipt reconciliation, cancellation and cleanup outcomes, opaque naming, restart, corruption and oversize rejection, and stale-writer CAS.
...
On completion of #87, [#88](https://github.com/SolusQuest/agentic-pr-review/issues/88) receives the Host state integration interface, central limits authority, runtime/test project ownership, state/session documentation status, and public-safe helpers needed for the two-fresh-process executable and Native AOT proof. #88 owns the synthetic transport oracle, final runtime CI replacement, and comparison-fixture deletion. R4 owns production GitHub Actions authorization, artifact/cache/object transport, Node bridge, key provisioning, retention jobs, and publication.
