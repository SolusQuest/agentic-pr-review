...
The checked V2 receipt and provenance remain immutable historical evidence for source commit `edc594c29a8a6b5fdacfab48643bf221277af200` and tree `8bf475a02a4f7307cdce2bbc29dd2bc6c6cf9089`. The current `trusted-proof-payload-v2` Runtime CI job instead selects the exact pull-request head SHA or triggering push SHA and runs `verify-r4-trusted-proof-payload-v2.sh` twice from that checkout. Each invocation independently verifies its own executable, verifier, receipt, and derived build-pair identities against the exact selected source commit and tree. The job does not compare the two executables, verifiers, complete receipts, or derived build-pair hashes with one another or with the checked historical receipt; the shared cross-build authority is the selected source and semantic contract, while per-build Native AOT bytes remain evidence for only that build.

Each receipt separately binds production native/managed identity and `production_payload_smoke=passed`, verifier native/managed identity and bounded evidence with `synthetic_native_aot_route=passed`, and `standalone_default_github=not_executed_e4_owned`. The two clean builds share exact source commit/tree, the immutable #179 receipt anchor, Action metadata and bundle, final topology and public resolver, deterministic provider, proof control/stale coordinator, trusted policy, preparation contract, toolchain policy, warning policy, and receipt schema. Each receipt retains its own executable, verifier, managed-input, and derived build-pair evidence; those per-build values are not cross-build continuity fields. #180 consumes and prepares only the production payload; it neither builds nor invokes the verifier. #181 owns the first separately authorized successful execution of the standalone production payload through its production GitHub/state/publisher composition against real GitHub while retaining the receipted deterministic provider; it does not claim a live DeepSeek transport run.

The historical #216 final receipt could not be checked into the commit it identified, so #216 completion required its recorded merge-SHA `push` Runtime CI to produce the then-defined identical receipts and record the canonical JSON bytes plus their SHA-256 on that issue. E3 [#180](https://github.com/SolusQuest/agentic-pr-review/issues/180) copied those immutable historical bytes to `runtime/tests/fixtures/action-host/trusted-proof/trusted-proof-payload-receipt.json` in its control root. That record is not current authority for equality between future clean builds. The E3 workflow checks out its exact control root and separate payload source root, then runs `prepare-r4-trusted-proof-payload.sh`; preparation never fetches an issue comment, mutable branch, release, artifact, or network receipt. The dependency chain is `#179 -> #216 -> #180 -> #181`.
...
`runtime/tests/fixtures/action-host/trusted-proof/**` is the closed E3 operation contract. A normal fixture PR and a separate all-`7` stale fixture PR both use the exact authorized default-branch workflow commit as their effective reviewed base and inherit its full tree. GitHub's historical reported PR base remains stable observed platform metadata; default/v1 uses that reported value as its effective base, while trusted-v2 uses the workflow SHA and never treats the historical PR-files view as change authority. The canary must be absent from the effective base. At the #181 one-time baseline, both fixture PRs are closed without merge and their operation refs are absent. The materializer recovers each last authenticated fixture head from the completed journal or immutable PR metadata, constructs a refreshed initial head with the exact main-plus-canary tree and ordered parents `[workflow commit, last authenticated fixture head]`, creates the absent exact ref once without force, and reopens the corresponding exact PR after readback. The stale advanced head is the only later ref update: a non-force fast-forward to a single-parent child of the refreshed stale head that changes only the canary. The evidence records every parent SHA rather than accepting boolean ancestry claims. Both normal runs keep one stable head. In the stale case, the already-authorized protected run reaches the provider signal and stale-ready readback before E4 advances only the canary, releases the run, and observes exact Host head revalidation reject the admitted head without candidate, sticky, or acceptance persistence. The separate workflow run caused by the advanced head must be observed queued without a job while the admitted run still owns the shared group, start only after that run completes, fail the old authorization before protected allocation, and terminate inertly.
...
Issue #181 authorizes one bounded maintainer-operated validation only after its final executable correction merges, all convergence gates pass, and the resulting default-branch commit and tree are read back unchanged. The earlier simplification merge alone does not activate execution authority. Known values are fixed before execution; GitHub-assigned IDs are discovered outputs and are used only after exact repository, workflow, fixture, head, operation, actor, and predecessor validation. Before any new credential-bearing work, exact named interrupted residue is removed, every prior run is terminal, relevant root/scoped artifacts and proof-control/product comments are absent, the authorization variable and proof-secret inventory are empty, and the protected-environment policy matches its unchanged baseline. Ambiguous or unreadable state stops the operation.

The operator first runs the exact-main Linux x64 Native AOT production-shaped synthetic route. While authorization and proof secrets remain absent, the operator confirms that PRs #225 and #226 are closed without merge and their operation refs are absent. The deterministic materializer recovers the last authenticated fixture heads from the completed journal or immutable PR metadata, creates each absent exact ref once at the corresponding exact-main-plus-canary descendant with ordered parents `[exact main, last authenticated fixture head]`, and reopens each exact PR after readback. The stale advanced commit remains a single-parent child of the refreshed stale head and is the only later non-force fast-forward. Their automatic trusted-proof runs drain inert. A fresh state key and deterministic-provider canary are then installed. The normal manifest proves bootstrap through its workflow-created outer ready plus maintainer release, followed by continuation through `verify-completed` and accepted prior SESSION. The stale manifest proves a separate outer ready/release pair, the inner stale-ready/stale-release pair, exact-head `stale_head` rejection with `state_disposition=not_committed`, no product publication, and an advanced-head preflight rejection before protected work.

A separate local Native AOT `LiveAgentVerifierFixture`, built from the same main commit, must complete its secret-free prepare and prepared-verification stages before performing exactly one secret-bearing production-DeepSeek diagnostic. It is not the E4 payload and makes no E4 live-provider claim. Cleanup removes only runtime-authenticated operation-owned resources, proves the environment policy remained unchanged and temporary names are absent, closes the two fixtures, and leaves workflow runs as the audit trail. The final issue comment contains only the tested main SHA, validated trusted-run URLs, bounded step outcomes, cleanup, and overall result. Outcome-unknown cleanup prevents a completion claim; one same-run infrastructure-only re-run is allowed only before product side effects, while product failures and the local diagnostic are never automatically retried.
