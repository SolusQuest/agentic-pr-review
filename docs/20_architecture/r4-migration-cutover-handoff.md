...
`runtime/tests/fixtures/action-host/trusted-proof/**` is the closed E3 operation contract. A normal fixture PR and a separate all-`7` stale fixture PR both use the exact authorized default-branch workflow commit as their base and inherit its full tree. The canary must be absent from the base; each initial head is a direct child adding only the regular LF-terminated `proof/apr178-path-canary.txt` blob. Both normal runs keep one stable head. The stale head advances once by modifying only that canary, and the old authorization must leave every follow-on run outside protected allocation and product state.

The dedicated operation admits only a completely enumerated empty relevant R4 artifact pre-state. E4 must stop before state-key resolution or provider construction when any locator-root, scoped, transaction, unknown, unauthenticated, or incompletely enumerated artifact exists. It records every operation-created artifact, deletes the exact operation-created root and scoped records after all runs settle, and requires complete re-enumeration to reproduce the empty pre-state before removing the state key or publishing evidence. The host-restricted schema retains exact run-to-state joins; the offline projector emits only the separate closed public-safe representation after cleanup. It performs no network request or mutation, and no real evidence instance is part of E3.
