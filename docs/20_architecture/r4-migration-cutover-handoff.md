...
`runtime/scripts/verify-r4-trusted-proof-payload.sh` publishes the proof payload for native `linux-x64`, exports the proof and Runtime managed inputs supplied to ILC, audits the proof-specific warning and architecture contracts, executes the provider-secret-free synthetic proof suite, and emits one canonical `APR_R4_E2P_RECEIPT`. Runtime CI invokes it twice in fresh roots on `ubuntu-24.04` and requires byte-identical receipt lines. The receipt binds the exact source commit/tree, the immutable #179 receipt anchor, Action metadata and bundle, payload and managed inputs, build pair, final topology and public resolver, deterministic provider, proof control/stale coordinator, trusted policy, preparation contract, toolchain, warning policy, and receipt schema.
