...
The operator first runs the exact-main Linux x64 Native AOT production-shaped synthetic route. While authorization and proof secrets remain absent, the deterministic materializer refreshes PRs #225 and #226 to commits with the exact-main-plus-canary tree and ordered parents `[exact main, exact live fixture-ref head read before mutation]`, using non-force fast-forward updates; the stale advanced commit remains a single-parent child of the refreshed stale head. Their automatic trusted-proof runs drain inert. A fresh state key and deterministic-provider canary are then installed. The normal manifest proves bootstrap through its workflow-created outer ready plus maintainer release, followed by continuation through `verify-completed` and accepted prior SESSION. The stale manifest proves a separate outer ready/release pair, the inner stale-ready/stale-release pair, exact-head `stale_head` rejection with `state_disposition=not_committed`, no product publication, and an advanced-head preflight rejection before protected work.
