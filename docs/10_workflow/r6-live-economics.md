...
Each child receives eight model calls and eight complete uniform per-call reservations. The default preparation uses the unchanged 4096 output-token adapter and current Agent limits. An explicitly authored experimental [#303](https://github.com/SolusQuest/agentic-pr-review/issues/303) plan may select either the retained closed 8192 adapter or the separate closed 65536 adapter, each with its matching limits identity and exact per-call output reservation. Their full child output/combined allocations are 65536/327680 and 524288/786432 tokens respectively; the input basis cannot exceed 32768 per call. Parent, child, restored state and journal all use the selected identity. Crossed adapter, configuration and reservation pairs reject before credential or worker launch. The 64K request output ceiling retains existing content, continuation, request/response, SESSION, state and timeout caps; an envelope failure is reported rather than silently increasing those caps. Smaller independent child quotas remain unsupported: their exhaustion would have a different meaning from T2's campaign-wide `budget_refused` event. Both experimental profiles are test-only and do not change the product default or authorize a paid economics campaign.
