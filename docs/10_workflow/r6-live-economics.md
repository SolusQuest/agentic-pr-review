...
Each child receives eight model calls and eight complete uniform per-call reservations. The default preparation uses the unchanged 4096 output-token adapter and current Agent limits. An explicitly authored experimental [#303](https://github.com/SolusQuest/agentic-pr-review/issues/303) plan may select the closed 8192 adapter with its matching limits identity and exact 8192 output-token reservation. Its full child output/combined allocation is 65536/327680 tokens; the input basis cannot exceed 32768 per call. Parent, child, restored state and journal all use the selected identity. Crossed adapter, configuration and reservation pairs reject before credential or worker launch. Smaller independent child quotas remain unsupported: their exhaustion would have a different meaning from T2's campaign-wide `budget_refused` event. This candidate is test-only and does not change the product default or authorize a paid economics campaign.
