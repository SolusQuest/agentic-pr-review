...
Population admission also preserves R5's case-definition invariant: within each side, one `(corpus_sha256, case_id)` must have one `(case_sha256, expected_defects)` definition, including failed outcomes. Mirroring a contradiction on the other side does not make it valid. Two internally consistent sides with different case definitions remain valid but not comparable. Every non-null execution digest must identify one attempt digest throughout the supplied pair, as the producer's execution hash includes its attempt identity. Exact self-comparison and distinct executions of the same planned attempt are allowed; attributing one execution to different attempts rejects before any effectiveness credit.
...
Both T3 pricing views, known subtotals, missing-usage/pricing coverage, reference applicability, execution-time unknowns and conservative reservations remain in the report. Same-token all-miss repricing is hypothetical, not another run, a measured saving or a cache-disabled control. A complete zero-traffic amount does not make unattempted work completed. Campaign completion rates always use the full admitted scheduled denominator and round to at most six fractional places; the counts remain authoritative. Missing evaluation files cannot discard failures or unattempted slots, or their known costs.

Execution completeness comes from finalized admitted attempt facts. A `caller_cancelled` or `deadline` stop observed after every scheduled attempt completed does not rewrite completed work as unfinished or suppress an otherwise supported descriptive comparison. The journal retains its stop cause, and Markdown displays it separately. Stops that leave failed or unattempted work still produce incomplete execution; unknown usage and pricing remain independently incomplete.
