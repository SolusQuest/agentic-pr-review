...
- **Provider** accepts the fixed `deepseek`/`deepseek-v4-flash` identity with one of three closed internal adapter profiles. The existing 4096 profile remains the default and retains its exact descriptor, adapter ID and Agent limits digest. The experimental 8192 and 65536 profiles each have distinct adapter IDs and limits digests; `configuration_sha256` must match the selected adapter. The example above is the unchanged 4096 default. Neither experimental profile can be selected through the Action or Host configuration or through generic replay admission. The evaluator's `r5-plan --profile output65536` selector remains internal to a separately reviewed plan.
...
- **Bounds** must all be positive and consistent: total ceilings cannot exceed `expanded evaluations ×` the selected Agent limits, per-call values must fit their totals, and the spend ceiling must cover one per-call charge. The default profile permits at most 4096 output tokens per call and retains cumulative Agent output/combined limits of 32768/294912. The experimental 8192 and 65536 profiles require exact output reservations of 8192 and 65536 per call, with cumulative Agent output/combined limits of 65536/327680 and 524288/786432 respectively; input remains 262144 and the model-call limit remains eight. An admitted plan may fund fewer than all possible sends and then stop at `bound_stop`. The 64K value is a request output ceiling under the existing content, continuation, request/response, SESSION, state and timeout caps, not a guarantee that eight arbitrary 64K reasoning turns can be retained and replayed. Both experimental profiles are evaluator-only under [#303](https://github.com/SolusQuest/agentic-pr-review/issues/303); neither is a product default or a live-spend grant.
