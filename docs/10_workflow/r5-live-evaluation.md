...
- **Provider** accepts the fixed `deepseek`/`deepseek-v4-flash` identity with one of two closed internal adapter profiles. The existing 4096 profile remains the default and retains its exact descriptor, adapter ID and Agent limits digest. The experimental 8192 profile has a distinct adapter ID and limits digest. `configuration_sha256` must match the selected adapter. The example above is the unchanged 4096 default; the candidate cannot be selected through the Action or Host configuration.
...
- **Bounds** must all be positive and consistent: total ceilings cannot exceed `expanded evaluations ×` the selected Agent limits, per-call values must fit their totals, and the spend ceiling must cover one per-call charge. The default profile permits at most 4096 output tokens per call and retains cumulative Agent output/combined limits of 32768/294912. The experimental profile requires an exact 8192 output-token reservation per call and uses cumulative Agent output/combined limits of 65536/327680; input remains 262144 and the model-call limit remains eight. An admitted plan may fund fewer than all possible sends and then stop at `bound_stop`. The 8192 profile is an evaluator-only experiment under [#303](https://github.com/SolusQuest/agentic-pr-review/issues/303), not a product default or a live-spend grant.
