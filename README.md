...
| `state_artifact_run_id`                            | empty          | Optional explicit run id; it must still match trusted workflow/event/head-repository, target-PR association, and manifest provenance checks. Pull-request incremental restore is limited to `pull_request` events     |
