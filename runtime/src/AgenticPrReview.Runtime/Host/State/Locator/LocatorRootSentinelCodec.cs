...
internal static class LocatorRootSentinelCodec
{
        ...
        out LocatorRootSentinel? sentinel)
    {
        ...
        try
        {
            ...
            if (key.Length != LocatorRootFormat.KeyBytes ||
                !StringComparer.Ordinal.Equals(
                ...
                !TryBuildAad(parsed.Header, repositoryId, out aad))
            {
                return false;
            }

            plaintext = new byte[parsed.Ciphertext.Length];
            using (var aes = new AesGcm(key, LocatorRootFormat.TagBytes))
            {
                aes.Decrypt(
                    parsed.Nonce,
                    parsed.Ciphertext,
