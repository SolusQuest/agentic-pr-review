...
public sealed class LedgerFixtureGeneratorTests
{
    ...
    public void GeneratorReproducesCheckedInFixturesByteForByte()
    {
        ...
        try
        {
            ...
            if (!proc.WaitForExit(180_000))
            {
                ...
                Assert.Fail("fixture generator did not exit within 180s");
            }
            var stderr = proc.StandardError.ReadToEnd();
            var stdout = proc.StandardOutput.ReadToEnd();
            Assert.True(proc.ExitCode == 0, $"generator exited {proc.ExitCode}. stderr={stderr}. stdout={stdout}");
    ...
    public void GeneratorRunIsIdempotent()
    {
        ...
        // Run twice against the same output directory; the second pass must not
        // change any generated byte.
        var single = Path.Combine(Path.GetTempPath(), "ledger-fixture-gen-single-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            // Snapshot the first pass, then run again against the same output
            // directory and verify no byte changed.
            var firstPassHashes = Directory.GetFiles(a).ToDictionary(f => Path.GetFileName(f)!, HashFile);
            Run(genProject, projectRoot, a);
            foreach (var (name, hash) in firstPassHashes)
            {
                Assert.Equal(hash, HashFile(Path.Combine(a, name)));
            }
    ...
    private static void Run(string genProject, string workingDir, string outDir)
    {
        ...
        {
            try { proc.Kill(entireProcessTree: true); } catch { }
            Assert.Fail("generator did not exit within 180s");
        }
        var stderr = proc.StandardError.ReadToEnd();
        var stdout = proc.StandardOutput.ReadToEnd();
        Assert.True(proc.ExitCode == 0, $"generator exited {proc.ExitCode}. stderr={stderr}. stdout={stdout}");
