...
public sealed partial class ActionHostCompositionTests
{
    ...
    private static AcceptedStateRecordSnapshot ReadAcceptedStateRecords(
        ...
        TimeProvider time,
        System.Action<StateControlHeaderV1, byte[], OpaqueStoreObjectMetadata>? observe = null,
        System.Action<LocatorContext, AuthorizedLocatorAccess>? inspectLocator = null)
    {
        ...
        try
        {
                ...
                !ReferenceEquals(item, rootMetadata)))
            {
                ...
                try
                {
                    var authenticated = Assert.IsType<StateControlHeaderV1>(
                        header);
                    observe?.Invoke(authenticated, payload, metadata);
            ...
            inspectLocator?.Invoke(context, access);
