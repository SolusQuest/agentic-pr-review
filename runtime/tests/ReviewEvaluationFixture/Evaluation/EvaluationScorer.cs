...
internal sealed record EvaluationOutcome(
    ...
    string? ConfigurationSha256, string? ExecutionSha256,
...
internal static class EvaluationScorer
{
    ...
    {
        if (subject is null) return Failure(testCase, EvaluationFailure.Invalid, EvaluationCode.SubjectInvalid);
        var spec = testCase.Input;
        if (spec.ReviewedIdentity.Runtime != subject.ReviewedIdentity)
            return Invalid(testCase, subject, EvaluationCode.WrongSnapshot);
        foreach (var required in spec.RequiredObservations)
        {
            if (!subject.Observations.Any(o => o.Tool == required.Tool))
                return Invalid(testCase, subject, EvaluationCode.RequiredToolMissing);
            if (!subject.Observations.Contains(required))
                return Invalid(testCase, subject, EvaluationCode.RequiredObservationMissing);
        }
        ...
        if (adjudication is not null)
        {
            if (!adjudication.Valid || adjudication.CorpusSha256 != spec.CorpusSha256 ||
                ...
                return Invalid(testCase, subject, EvaluationCode.AdjudicationInvalid);
        ...
        return new(spec.Id, spec.CorpusSha256, testCase.Sha256, subject.ConfigurationSha256, subject.ExecutionSha256,
    ...
    internal static EvaluationOutcome Failure(EvaluationCase testCase, EvaluationFailure failure,
        ...
        new(testCase.Input.Id, testCase.Input.CorpusSha256, testCase.Sha256, null, null, null, null, null, null,
    ...
    private static EvaluationOutcome Invalid(EvaluationCase testCase, EvaluationSubject subject, EvaluationCode code) =>
        Failure(testCase, EvaluationFailure.Invalid, code) with
        {
            ConfigurationSha256 = subject.ConfigurationSha256,
            ExecutionSha256 = subject.ExecutionSha256,
            SourceCommit = subject.Run.SourceCommit,
            SourceTree = subject.Run.SourceTree,
            SourceClean = subject.Run.SourceClean,
            Mode = subject.Run.Mode,
