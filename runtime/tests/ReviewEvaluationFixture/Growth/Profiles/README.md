...
`tool_observations` counts actual `AgentToolResultEvent` results from the shared replay executor. It excludes `finish_review`, which emits a call event but no result event, and does not claim to count all tool calls. A completed finish-only attempt therefore has zero observations; completed tools, continuation and updates attempts have eight, one and one respectively. Failed attempts retain the observed result count when the child reply is admitted, including known zero for cancellation before execution; an unadmitted reply leaves the count null.
