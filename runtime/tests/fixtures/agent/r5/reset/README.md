...
Both emit `r5-reset-owner-v1`; record the launch command to distinguish framework from actual Native AOT execution. The native probe covers those four owner cases, not the complete capacity/fault matrix. All inputs are synthetic and no real GitHub mutation or provider call occurs.
