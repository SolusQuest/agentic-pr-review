...
The `live` mode is not a contributor-facing provider command. `.github/workflows/r3-live-proof.yml` is its only credential-bearing caller: a no-input manual workflow on an exact merged `main` commit. The workflow proves repository, workflow, checkout, ancestry, clean-tree, and prepared-build identity before mapping the dedicated environment secret only to the final supervisor step. That supervisor replaces its shell immediately, clears the ambient credential, creates one in-memory 32-byte STATE key, and launches the same prepared verifier artifact in four serial fresh processes for must-find, must-not-find, continuation seed, and continuation restore. It emits one bounded JSON completion line only after child termination, sensitive-tree cleanup, and owned-key zeroing. The retired direct DeepSeek compatibility probe is not an alternate route.
