...
fail() {
  echo "APR_R4_E2P_PREPARATION_V2_INVALID $1" >&2
  exit 1
}
...
expected_receipt="$control_root/runtime/tests/fixtures/action-host/trusted-proof/trusted-proof-payload-receipt-v2.json"
...
receipt_kind="$(node --input-type=module - "$receipt" <<'NODE'
import fs from 'node:fs';
const value = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));
if (value.kind !== 'apr-r4-e2p-trusted-proof-payload-v2' ||
    value.action_source_sha !== '5b5769753653bb3fd3e68cf8b7bb88a1bd350613' ||
    value.compiled_payload_source_commit !== value.source_commit ||
    value.compiled_payload_source_tree !== value.source_tree) process.exit(1);
...
dotnet publish "$project" --configuration Release --runtime linux-x64 \
  ...
  "-p:PayloadSourceCommit=$source_commit" "-p:PayloadSourceTree=$source_tree" \
...
control_checker="$control_root/scripts/check-r4-e2p-receipt-v2.mjs"
[[ -f "$control_checker" && ! -L "$control_checker" ]] || fail control-checker
[[ "$(realpath -e "$control_checker")" == "$control_checker" ]] ||
  fail control-checker-path
