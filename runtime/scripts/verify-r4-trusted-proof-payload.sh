...
emit_safe_supervisor_failure() {
  local evidence="$1"
  if [[ ! -f "$evidence" ]]; then
    printf 'APR_R4_E2P_FAILURE stage=synthetic-route case=supervisor code=no-evidence status=failed\n' >&2
    return
  fi
...
if (failed.length === 0) {
  process.stderr.write('APR_R4_E2P_FAILURE stage=synthetic-route case=supervisor code=invalid-evidence status=failed\n');
} else {
  for (const item of failed.slice(0, 3)) {
    ...
    process.stderr.write(`APR_R4_E2P_FAILURE stage=synthetic-route case=${id} code=case-failed status=${status}\n`);
  }
}
NODE
}
...
execute_proof() {
  ...
  [[ -x "$payload" && ! -e "$payload.dll" && ! -e "$payload.deps.json" ]] ||
    return 1
  run_production_smoke || {
    echo 'APR_R4_E2P_FAILURE stage=production-smoke case=aggregate code=smoke-failed status=failed' >&2
    return 1
  }
...
NODE
  ...
  printf '{"source_commit":"%s","source_tree":"%s","payload_sha256":"%s","proof_managed_intermediate_sha256":"%s","runtime_managed_intermediate_sha256":"%s","managed_architecture_sha256":"%s","verifier_sha256":"%s","verifier_managed_intermediate_sha256":"%s","verifier_payload_managed_intermediate_sha256":"%s","verifier_runtime_managed_intermediate_sha256":"%s","verifier_managed_architecture_sha256":"%s","verifier_evidence_sha256":"%s","build_pair_sha256":"%s"}\n' \
    ...
    "$verifier_evidence_sha" "$pair_sha" > "$identity"
